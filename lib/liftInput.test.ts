...
test("a good day comes back parsed", () => {
  const r = parse(day({ "back-squat": [{ w: 225, r: 5 }, { w: 245, r: 3 }] }));
...
test("a lift has to be a list of sets", () => {
  assert.equal(parse(day({ "back-squat": "225x5" })).ok, false);
  assert.equal(parse(day({ "back-squat": { w: 225, r: 5 } })).ok, false);
  assert.equal(parse(day({ "back-squat": [225] })).ok, false, "a bare number is not a set");
});

test("more sets than we record is refused rather than truncated", () => {
  const many = Array.from({ length: MAX_SETS + 1 }, () => ({ w: 135, r: 5 }));
  const r = parse(day({ "back-squat": many }));
  assert.equal(r.ok, false);
  assert.match(r.error!, new RegExp(String(MAX_SETS)));
  assert.equal(parse(day({ "back-squat": many.slice(1) })).ok, true, "and one fewer is fine");
});
...
test("reps have to be a whole number inside the range", () => {
  for (const r of [0, -1, 2.5, MAX_REPS + 1, "five"])
    assert.equal(parse(day({ "back-squat": [{ w: 225, r }] })).ok, false, String(r));
  assert.equal(parse(day({ "back-squat": [{ w: 225, r: MAX_REPS }] })).ok, true);
});

test("a weight outside the range is refused", () => {
  for (const w of [-5, MAX_WEIGHT + 1, "heavy"])
    assert.equal(parse(day({ "back-squat": [{ w, r: 5 }] })).ok, false, String(w));
  assert.equal(parse(day({ "back-squat": [{ w: MAX_WEIGHT, r: 1 }] })).ok, true);
});

test("half-pound microplates survive; anything finer is rounded off", () => {
  const r = parse(day({ "back-squat": [{ w: 102.5, r: 5 }, { w: 100.37, r: 5 }] }));
  assert.deepEqual(r.value!.lifts["back-squat"], [
    { w: 102.5, r: 5 },
    { w: 100.5, r: 5 },
  ]);
});
...
test("a weight with no reps is refused, and the message says which set", () => {
  const r = parse(day({ "back-squat": [{ w: 225, r: 5 }, { w: 245, r: "" }] }));
  assert.equal(r.ok, false);
  assert.match(r.error!, /set 2 of Back squat/i);
  assert.match(r.error!, /no reps/i);
});

test("reps with no weight are refused on a loaded lift", () => {
  const r = parse(day({ "bench-press": [{ w: "", r: 5 }] }));
  assert.equal(r.ok, false);
  assert.match(r.error!, /no weight/i);
});

test("reps with no weight are a bodyweight set on a bodyweight lift", () => {
  const r = parse(day({ "chin-up": [{ w: "", r: 8 }, { w: 25, r: 5 }] }));
  assert.equal(r.ok, true, r.error);
  assert.deepEqual(r.value!.lifts["chin-up"], [
    { w: 0, r: 8 },
    { w: 25, r: 5 },
  ]);
});
...
test("zero on the bar is refused on a loaded lift", () => {
  assert.equal(parse(day({ "bench-press": [{ w: 0, r: 5 }] })).ok, false);
});
...
test("a row the athlete tabbed through and left empty is not an error", () => {
  const r = parse(
    day({ "back-squat": [{ w: 225, r: 5 }, { w: "", r: "" }, { w: null, r: null }] }),
  );
  assert.equal(r.ok, true, r.error);
  assert.deepEqual(r.value!.lifts["back-squat"], [{ w: 225, r: 5 }]);
});

test("a lift whose rows were all left blank is not part of the day", () => {
  const r = parse(day({ "back-squat": [{ w: 225, r: 5 }], "bench-press": [{ w: "", r: "" }] }));
  assert.equal(r.ok, true, r.error);
  assert.deepEqual(Object.keys(r.value!.lifts), ["back-squat"]);
});

test("a day with nothing in it is refused rather than stored empty", () => {
  assert.equal(parse(day({})).ok, false);
  assert.equal(parse(day({ "back-squat": [] })).ok, false);
  assert.equal(parse(day({ "back-squat": [{ w: "", r: "" }] })).ok, false);
...
test("a long note is trimmed rather than refused", () => {
  const r = parse(day({ "back-squat": [{ w: 225, r: 5 }] }, "x".repeat(3000)));
