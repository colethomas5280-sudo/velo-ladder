...
Expected: `200` with a `facility` board containing the seeded Jordan Hale pull-down; both leak checks `false`; the bad tracker returns `400`; both archived checks print `true`.
...
# and GET /api/leaderboard?tracker=pulldown&oz=5 — Jordan Hale should now appear
