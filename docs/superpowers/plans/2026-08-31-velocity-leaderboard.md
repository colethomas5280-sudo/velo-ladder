...
Expected: `200` with a `facility` board containing the seeded Jordan Hale pull-down; both leak checks `false`; the bad tracker returns `400`; the archived check prints `true`.
