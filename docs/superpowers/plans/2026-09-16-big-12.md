...
- [ ] **Step 5: Write them in `upsertScreen`**
...
Run: `npx tsx --test lib/dataRows.test.ts`
Expected: PASS. `tsc --noEmit` will now fail in `lib/screenInput.ts` because `ScreenInput` has no `flaws`; Task 5 fixes that. Do not commit until it does.
