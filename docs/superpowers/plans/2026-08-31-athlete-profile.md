...
test("splitName splits on the LAST space, so compound given names survive", () => {
  assert.deepEqual(splitName("Jordan Hale"), { first: "Jordan", last: "Hale" });
...
test("joinName is the only way name is built, and trims a missing half", () => {
  assert.equal(joinName("Jordan", "Hale"), "Jordan Hale");
  assert.equal(joinName("Prince", ""), "Prince");
  assert.equal(joinName("", "Hale"), "Hale");
});

test("an athlete never receives coach_notes", () => {
  const row = { id: "a1", name: "Jordan Hale", phone: "555", coachNotes: "shoulder concern" };
...
test("a coach receives every field", () => {
  const row = { id: "a1", name: "Jordan Hale", coachNotes: "shoulder concern" };
...
test("missing counts only the fields a coach would actually chase", () => {
  const full = {
    firstName: "Jordan", lastName: "Hale", phone: "555-0100",
...
Expected: `"schemaVersion":14`, `"missing":[]`. Then confirm the seeded athlete backfilled — `GET /api/athletes/seed-md` with a coach cookie should show `"firstName":"Jordan","lastName":"Hale"`. Run setup a second time and confirm the backfill does not re-run over an edited name.
