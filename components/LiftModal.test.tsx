...
test("picking a lift opens it with one empty set", () => {
  open();
  addLift("front-squat");
  assert.ok(screen.getByText("Front squat"));
...
test("a lift already in the day is not offered again", () => {
  ...
  addLift("front-squat");
  const options = [...document.querySelectorAll(".lm-add option")].map((o) => o.textContent);
  assert.equal(options.includes("Front squat"), false);
  assert.ok(options.includes("Bench"));
});
...
test("a lift shows what was hit last time and the best ever", () => {
  open([
    day(daysAgo(30), { "front-squat": [{ w: 225, r: 5 }] }),
    day(daysAgo(7), { "front-squat": [{ w: 245, r: 3 }] }),
  ]);
  addLift("front-squat");
...
test("a lift never done before says so instead of showing a blank target", () => {
  open([day(daysAgo(7), { "bench": [{ w: 185, r: 5 }] })]);
  addLift("front-squat");
...
test("the day being edited is left out of its own past", () => {
  open(
    [
      day(daysAgo(7), { "front-squat": [{ w: 225, r: 5 }] }),
      day(TODAY, { "front-squat": [{ w: 315, r: 1 }] }),
    ],
    {
      id: "d1",
      athleteId: "a1",
      date: TODAY,
      lifts: { "front-squat": [{ w: 315, r: 1 }] },
      notes: "",
      level: null,
    },
  );
...
test("an existing day opens with its sets filled in", () => {
  open([], {
    ...
    athleteId: "a1",
    date: TODAY,
    lifts: { "front-squat": [{ w: 225, r: 5 }, { w: 245, r: 3 }] },
    notes: "felt good",
    level: null,
  });
...
test("a lift can be taken back out of the day", () => {
  open();
  addLift("front-squat");
  fireEvent.click(screen.getByLabelText(/remove front squat/i));
  assert.equal(document.querySelectorAll(".lm-lift").length, 0);
});
...
test("the best names the set its number came from, not the heaviest of the day", () => {
  open([day(daysAgo(1), { "front-squat": [{ w: 275, r: 3 }, { w: 245, r: 8 }] })]);
  addLift("front-squat");
  const target = document.querySelector(".lm-target")!.textContent!;
